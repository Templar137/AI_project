# -*- coding: utf-8 -*-
"""SC2 Cinematic remastering Code_Final
...
# shutil.rmtree(y_train_dir)720p_
...
while(vidcap.isOpened()):
   ...
   # 30프레임당 하나씩 이미지 추출 
   if(int(vidcap.get(1)) % 30 == 0 and 150 < count < 4650):  ## 이걸 만들어 둔 이유가 램부족떄문입니다!
     cv2.imwrite("/content/x_train/%d.jpg" % count, image)
   elif(int(vidcap.get(1)) % 100 == 10 and 150 < count < 4650): 
...
for i in name_x_train:
  ...
  img = cv2.pyrDown(img)
  x_train.append(img_to_array(Image.fromarray(img)))
num = len(x_train)
for i in range(num):
...
def RDNv2(input_shape = (None, None, 3)):
    def upsample(x):
        x = tf.concat([x, x, x, x], axis=-1)
        x = tf.nn.depth_to_space(x, 2)
    ...
    # RDN n_block = 5
    Dense_block_1 = RDB(x)
    block_1_up = tf.keras.layers.Conv2DTranspose(32, (3, 3), strides = (2, 2), activation = None, padding = 'same')(Dense_block_1)
    Dense_block_2 = RDB(Dense_block_1)
    block_2_up = tf.keras.layers.Conv2DTranspose(32, (3, 3), strides = (2, 2), activation = None, padding = 'same')(Dense_block_2)
    Dense_block_3 = RDB(Dense_block_2)
    block_3_up = tf.keras.layers.Conv2DTranspose(32, (3, 3), strides = (2, 2), activation = None, padding = 'same')(Dense_block_3)
    Dense_block_4 = RDB(Dense_block_3)
    block_4_up = tf.keras.layers.Conv2DTranspose(32, (3, 3), strides = (2, 2), activation = None, padding = 'same')(Dense_block_4)
    Dense_block_5 = RDB(Dense_block_4)
    block_5_up = tf.keras.layers.Conv2DTranspose(32, (3, 3), strides = (2, 2), activation = None, padding = 'same')(Dense_block_5)
    ...
    conv_total = Conv2D(n_filters, (3, 3), activation=None, padding="same",  activity_regularizer=regularizers.l1(l1_alpha))(conv_1)

    upsample_transpose = tf.keras.layers.Conv2DTranspose(32, (3, 3), strides = (2, 2), activation = None, padding = 'same')(conv_total)
    upsample = tf.keras.layers.Concatenate()([upsample_transpose, block_1_up, block_2_up, block_3_up, block_4_up, block_5_up])
    upsample = Conv2D(n_filters, (1, 1), activation=None, padding="same",  activity_regularizer=regularizers.l1(l1_alpha))(upsample)
...
callbacks_list = [
    tf.keras.callbacks.ModelCheckpoint(
        filepath = 'models/RDN_model_25e-5_learning_free_size_lotv_improved.h5',
        monitor = 'val_loss',
        save_best_only=True
    )]

history = model.fit(x_train, y_train, validation_data = (x_test, y_test), batch_size = 4, epochs = 300, callbacks = callbacks_list)
# Epoch 254/300
...
model = tf.keras.models.load_model('/content/models/RDN_model_25e-5_learning_free_size_lotv_improved.h5', custom_objects = {'psnr_loss': psnr_loss})/content/RDN_model_25e-5_learning_free_size_lotv_improved.h5

predict = model.predict(x_test, batch_size = 4)

predict_y = model.predict(y_test, batch_size = 1)
...
dependencies = {'psnr_loss': psnr_loss}
model = tf.keras.models.load_model('/content/gdrive/MyDrive/RDN_model_25e-5_learning_free_size_lotv_improved.h5', custom_objects = {'psnr_loss': psnr_loss})

os.mkdir('/content/video')
os.mkdir('/content/video_360')
os.mkdir('/content/video_bicubic')
os.mkdir('/content/video_remaster')

os.mkdir('/content/video_zeratul_remaster')
os.mkdir('/content/video_zeratul')
os.mkdir('/content/video_zeratul_bicubic')
os.mkdir('/content/video_zeratul_360')

shutil.rmtree('/content/video_bicubic')

vidcap = cv2.VideoCapture('/content/gdrive/MyDrive/Dataset/SC2 Super Resolution/lotv_720p_24fps.mp4') 
count = 0 
while(vidcap.isOpened()):
   ret, image = vidcap.read()
   cv2.imwrite("/content/video/%d.jpg" % count, image) 
   print('Saved frame%d.jpg' % count) 
   count += 1 
   if count == 4609:
     break
...
vidcap = cv2.VideoCapture('/content/gdrive/MyDrive/Dataset/SC2 Super Resolution/Lotv_360_24fps.mp4') 
count = 0 
while(vidcap.isOpened()):
   ret, image = vidcap.read()
   cv2.imwrite("/content/video_360/%d.jpg" % count, image) 
   print('Saved frame%d.jpg' % count) 
   count += 1 
   if count == 4609:
     break
...
vidcap = cv2.VideoCapture('/content/gdrive/MyDrive/Dataset/SC2 Super Resolution/zeratul_720p_30fps.mp4') 
count = 0 
while(vidcap.isOpened()):
   ret, image = vidcap.read()
   cv2.imwrite("/content/video_zeratul/%d.jpg" % count, image) 
   print('Saved frame%d.jpg' % count) 
   count += 1 
   if count == 7576:
     break
...
vidcap = cv2.VideoCapture('/content/gdrive/MyDrive/Dataset/SC2 Super Resolution/zeratul_360p_30fps.mp4') 
count = 0 
while(vidcap.isOpened()):
   ret, image = vidcap.read()
   cv2.imwrite("/content/video_zeratul_360/%d.jpg" % count, image) 
   print('Saved frame%d.jpg' % count) 
   count += 1 
   if count == 7576:
     break
...
video = sorted(os.listdir('/content/video'))
...
for count in range(5000):
  ...
  for i in range(count * num, (count + 1) * num):
    img = cv2.cvtColor(cv2.imread('/content/video/' + str(i) + '.jpg'), cv2.COLOR_BGR2RGB)
    img = cv2.pyrDown(img)
    list_video.append(img_to_array(Image.fromarray(img)))
    k += 1
  list_video = np.array(list_video)
  list_video = list_video / 255.0
  predict = model.predict(list_video, batch_size = 2)
  for i in range(num):
    plt.imsave('/content/video_remaster/' + str(i + count * num) + '.jpg', predict[i])
  if count % 500 == 0:
    print(count)
...
for count in range(5000):
  ...
  list_video = list_video / 255.0
  for i in range(num):
    plt.imsave('/content/video_bicubic/' + str(i + count * num) + '.jpg', list_video[i])
  if count % 50 == 0:
    print(count)
...
num = 1
k = 0
for count in range(7576):
  list_video = []
  if count >= 7576:
    break
  for i in range(count * num, (count + 1) * num):
    img = cv2.cvtColor(cv2.imread('/content/video_zeratul/' + str(i) + '.jpg'), cv2.COLOR_BGR2RGB)
    img = cv2.pyrDown(img)
    list_video.append(img_to_array(Image.fromarray(img)))
  list_video = np.array(list_video)
  list_video = list_video / 255.0
  predict = model.predict(list_video, batch_size = 2)
  for i in range(num):
    plt.imsave('/content/video_zeratul_remaster/' + str(i + count * num) + '.jpg', predict[i])
  if count % 500 == 0:
    print(count)
...
list_video = []
num = 4
for count in range(1894):
  list_video = []
  for i in range(count * num, (count + 1) * num):
    img = cv2.cvtColor(cv2.imread('/content/video_zeratul_360/' + str(i) + '.jpg'), cv2.COLOR_BGR2RGB)
    # img = cv2.pyrDown(img)
    img = cv2.resize(img, (1280, 720), interpolation = cv2.INTER_CUBIC)
    list_video.append(img_to_array(Image.fromarray(img)))
  list_video = np.array(list_video)
  list_video = list_video / 255.0
  for i in range(num):
    plt.imsave('/content/video_zeratul_bicubic/' + str(i + count * num) + '.jpg', list_video[i])
  if count % 125 == 0:
    print(count)

os.chdir("/content/video_remaster/")

my_zip = zipfile.ZipFile("0.zip", 'w')
my_zip.close()

# Video Generating function
def generate_video(image_folder ='/content/video_remaster/', frame = 24):
    video_name = 'SC2_zeratul_Final.mp4'
    os.chdir(image_folder)
      
    images = [str(i) + '.jpg' for i in range(7576)]
  
    # setting the frame width, height width
    # the width, height of first image
  
    video = cv2.VideoWriter(video_name, cv2.VideoWriter_fourcc(*'DIVX'), frame, (1280, 720)) 
...
generate_video(image_folder = '/content/video_zeratul_bicubic/', frame = 30)
generate_video(image_folder = '/content/video_zeratul_remaster/', frame = 30)

file_ls = ['/content/video_zeratul_remaster/SC2_zeratul_Final.mp4']
with zipfile.ZipFile("/content/gdrive/MyDrive/SC2_zeratul_Final_remaster.zip", 'w') as my_zip:
    for i in file_ls:
        my_zip.write(i)
    my_zip.close()